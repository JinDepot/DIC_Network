...
function App() {
    ...
    useEffect(() => {
        ...
        let new_link = fullData.links.filter((link) => {
            return (
                (new_node_id_list.includes(link.source) &&
                    new_node_id_list.includes(link.target)) ||
                (new_node_id_list.includes(link.source.id) &&
                    new_node_id_list.includes(link.target.id))
            );
        });
    ...
    useEffect(() => {
        let top5MarketCap = data.nodes
            .sort((a, b) => {
                return b.market_cap_raw - a.market_cap_raw;
            })
            .slice(0, 5);

        let top5Degree = data.nodes
            .sort((a, b) => {
        ...
        <div className="App">
            <div className={`headerContainer`}>
                <span className={`header_title`}>
                    Data-Driven Industry Network
                </span>
                <div className={`header noselect`}>
                    <span className={`header_intro`}>
                        An industry classification scheme consists of a set of
                        categories, each of which groups financial entities such
                        as companies, publicly traded securities, or
                        establishments together. Entities contained in each
                        category must be of “similar” types of businesses given
                        their market behavior or operational structures, while a
                        category should be notably different from another.
                    </span>
                    <br />
                    <span className={`header_intro`}>
                        We introduce a Data-driven Industry Classification
                        scheme, DIC, namely, which detects firm relations from a
                        stock market and exresses them in a network structure,
                        with firms as nodes and the inter-firm similarity as
                        edges. The resulting network is then segmented by a set
                        of sub-graph clusters representing industry groups. We
                        apply various methods such as BERT, news curation, and
                        community detection to determine “business identities”
                        from corporate disclosures, while curating “market
                        perception” based on the contents of news articles) in
                        order to categorize firms that are considered similar.
                    </span>
                    <br />
                    <span className={`header_intro`}>
                        Please hover over each configuration option to see the
                        possible selections.
                    </span>
                </div>
            </div>
                ...
                {config.market !== "" ? (
                    <>
                        <div className={`tabSelector`}>
                            <div
                                className={`tab noselect ${
                                    config.viewSource === "10K"
                                        ? "selected"
                                        : ""
                                }`}
                                onClick={(e) => {
                        ...
                        >
                            <span className={`header_intro`}>
                                {tab === "10k"
                                    ? "Use 10Ks only to construct the industry network"
                                    : "Use 10Ks and news articles to construct the industry network"}
                            </span>
                        </div>
                    </>
                ) : null}
                {open ? (
                    <React.Fragment>
                        <div className={`networkSummaryContainer`}>
                            <a className={`summaryTitle`}>NETWORK SUMMARY</a>
                            ...
                            <a className={`summaryTitle`}>Top 5 Market Cap</a>
                            <div className={`summarySection`}>
                                <div className={`summaryContent`}>
                            ...
                            <a className={`summaryTitle`}>Top 5 Degrees</a>
                            <div className={`summarySection`}>
                                <div className={`summaryContent`}>
                        ...
                        <div className={`networkContainer`}>
                            {dimension ? (
                                <ForceGraph2D
                                    ...
                                    nodeColor={(node) =>
                                        node.selected
                                            ? generateColor(node.GICS_sector)
                                            : "rgba(225, 229, 233, 0.1)"
                                    }
                                    linkColor={(link) =>
                                        link.selected
                                            ? "rgba(137, 146, 155, 0.9)"
                                            : "rgba(225, 229, 233, 0.1)"
                                    }
                            ...
                            <div className={`networkInteractionBoardContainer`}>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <b>
                                            {" "}
                                            {config.market === "S&P500"
                                                ? "GICS"
                                                : "KOSPI"}{" "}
                                            &nbsp;{" "}
                                        </b>{" "}
                                        : {selected.GICS_sector}
                                    </a>
                                    <a
                                        style={{
                                            display: "flex",
